...
@setlocal
rem ********************************************************************************************
rem * Deployment script for the CPX Webclient
...
echo %LINE%
echo Start installing BSCS CPX Webclient
echo %LINE%
...
echo Start unpacking CPX files ... 

mkdir %WEBAPPS_DIR%\%APP_NAME%

chdir /D %WEBAPPS_DIR%\%APP_NAME%
"%JAVA_HOME%\bin\jar" xfM %BSCS_JAR%\cpx.zip
...
echo %LINE%
echo BSCS CPX Webclient installation done.
echo %LINE%
goto end
