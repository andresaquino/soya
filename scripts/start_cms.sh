...
# 
#  usage: 
#         start_cms.sh [-nomon] [-name <server_name>]
#
#  description:
...
#  examples:
#         start_cms.sh 
#         start_cms.sh -name my_server_name
#         start_cms.sh -nomon 
#         start_cms.sh -nomon -name my_server_name
# **************************************************** 
...
MYPATH=$MYPATH:$BSCS_JAR/cms_in.jar
MYPATH=$MYPATH:$BSCS_JAR/cms_external.jar

# add all plugins that are found for cms
if [ -d "$BSCS_JAR/cms_plugin" ] ; then
  for jarfile in `ls $BSCS_JAR/cms_plugin/*.jar` ; do  MYPATH=$MYPATH:$jarfile ; done
fi
...
# CMS_PORT=<port number>
# For the Sun JDK
# ORBSERVERPORT=" -Dcom.sun.CORBA.ORBServerPort="${CMS_PORT}
# For the IBM JDK
# ORBSERVERPORT=" -Dcom.ibm.CORBA.ListenerPort="${CMS_PORT}
